...
func (fa *ASTFunc) generate() {
	// TODO maybe name the return arguments ... because of clang_getDiagnosticOption -> the normal return can be always just "o"? https://github.com/zimmski/go-clang-phoenix/issues/57

	// TODO reenable this, see the comment at the bottom of the generate function for details https://github.com/zimmski/go-clang-phoenix/issues/54
...
func (fa *ASTFunc) generateParameters() []ast.Expr {
	...
	for _, p := range fa.f.Parameters {
		...
		if p.Type.IsSlice {
			...
			} else if p.Type.CGoName == "cxstring" { // TODO try to get cxstring and "String" completely out of this function since it is just a struct which can be handled by the struct code https://github.com/zimmski/go-clang-phoenix/issues/25
...
func (fa *ASTFunc) generateReturn(call ast.Expr) {
	...
	if returnType.GoName != "void" || len(fa.ret.Results) > 0 {
		if returnType.GoName == "cxstring" {
			...
			} else if returnType.CGoName == CSChar && returnType.PointerLevel == 1 { // TODO refactor the const char * check so that one function is used everywhere to check for that C type https://github.com/zimmski/go-clang-phoenix/issues/56
...
func (fa *ASTFunc) addEmptyLine() {
	// TODO this should be done using something else than a fake statement. https://github.com/zimmski/go-clang-phoenix/issues/53
	fa.addStatement(&ast.ExprStmt{
		X: &ast.CallExpr{
			Fun: &ast.Ident{
				Name: "REMOVE",
