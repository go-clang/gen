...
func (i Index) TranslationUnitFromSourceFile(source_filename string, clang_command_line_args []string, unsaved_files []UnsavedFile) TranslationUnit {
	ca_clang_command_line_args := make([]*C.schar, len(clang_command_line_args))
	var cp_clang_command_line_args **C.schar
	if len(clang_command_line_args) > 0 {
		cp_clang_command_line_args = &ca_clang_command_line_args[0]
	}
	...
	return TranslationUnit{C.clang_createTranslationUnitFromSourceFile(i.c, c_source_filename, C.int(len(clang_command_line_args)), cp_clang_command_line_args, C.uint(len(unsaved_files)), cp_unsaved_files)}
}
...
func (i Index) ParseTranslationUnit(source_filename string, command_line_args []string, unsaved_files []UnsavedFile, options uint16) TranslationUnit {
	ca_command_line_args := make([]*C.schar, len(command_line_args))
	var cp_command_line_args **C.schar
	...
	return TranslationUnit{C.clang_parseTranslationUnit(i.c, c_source_filename, cp_command_line_args, C.int(len(command_line_args)), cp_unsaved_files, C.uint(len(unsaved_files)), C.uint(options))}
}
